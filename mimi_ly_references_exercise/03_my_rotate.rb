...
def my_rotate!(array, amt)
    ...
    if amt > 0 #positive
        amt.times do 
            ele = array.shift()
            array.push(ele) #remove first element and move to last
        end
    else #negatve
        amt.abs.times do #change negative to positive
            ele = array.pop()
            array.unshift(ele) #remove last element and move to first
        end
    end
    array
end
